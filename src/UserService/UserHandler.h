...
void UserHandler::RegisterUserWithId(
  ...
  auto span = opentracing::Tracer::Global()->StartSpan("RegisterUserWithId", { opentracing::ChildOf(parent_span->get()) });
  ...
  if (mongoc_cursor_error (cursor, &error)) {
    ...
    started_mongo_find = std::chrono::high_resolution_clock::now();
    auto sucess = mongoc_collection_insert_one(collection, new_doc, nullptr, nullptr, &error);
    timetaken_mongo_call = std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::high_resolution_clock::now()-started_mongo_find).count();
    timetaken_by_external_calls = timetaken_by_external_calls + timetaken_mongo_call; 
...
void UserHandler::RegisterUser(
  ...
  TextMapWriter writer(writer_text_map);
  auto parent_span = opentracing::Tracer::Global()->Extract(reader);
  auto span = opentracing::Tracer::Global()->StartSpan("RegisterUser",{ opentracing::ChildOf(parent_span->get()) });
  opentracing::Tracer::Global()->Inject(span->context(), writer);
...
void UserHandler::UploadCreatorWithUsername(
  ...
  TextMapWriter writer(writer_text_map);
  auto parent_span = opentracing::Tracer::Global()->Extract(reader);
  auto span = opentracing::Tracer::Global()->StartSpan("UploadUserWithUsername", { opentracing::ChildOf(parent_span->get()) });
  opentracing::Tracer::Global()->Inject(span->context(), writer);
  ...
  if (memcached_client) {
    ...
    auto started_mongo_find = std::chrono::high_resolution_clock::now(); 
    user_id_mmc = memcached_get(memcached_client, (username+":user_id").c_str(), (username+":user_id").length(), &user_id_size, &memcached_flags, &memcached_rc);
    auto timetaken_mongo_call = std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::high_resolution_clock::now()-started_mongo_find).count();
    timetaken_by_external_calls = timetaken_by_external_calls + timetaken_mongo_call;  
...
void UserHandler::UploadCreatorWithUserId(
  ...
  TextMapWriter writer(writer_text_map);
  auto parent_span = opentracing::Tracer::Global()->Extract(reader);
  auto span = opentracing::Tracer::Global()->StartSpan("UploadUserWithUserId", { opentracing::ChildOf(parent_span->get()) });
  opentracing::Tracer::Global()->Inject(span->context(), writer);
...
void UserHandler::Login(
  ...
  TextMapWriter writer(writer_text_map);
  auto parent_span = opentracing::Tracer::Global()->Extract(reader);
  auto span = opentracing::Tracer::Global()->StartSpan("Login", { opentracing::ChildOf(parent_span->get()) });
  opentracing::Tracer::Global()->Inject(span->context(), writer);
  ...
  if (!memcached_client) {
    ...
    auto started_mongo_find = std::chrono::high_resolution_clock::now();  
    login_mmc = memcached_get(memcached_client, (username + ":login").c_str(), (username + ":login").length(), &login_size, &memcached_flags, &memcached_rc);
    auto timetaken_mongo_call = std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::high_resolution_clock::now()-started_mongo_find).count();
    timetaken_by_external_calls = timetaken_by_external_calls + timetaken_mongo_call; 
...
int64_t UserHandler::GetUserId(
  ...
  TextMapWriter writer(writer_text_map);
  auto parent_span = opentracing::Tracer::Global()->Extract(reader);
  auto span = opentracing::Tracer::Global()->StartSpan("GetUserId",{ opentracing::ChildOf(parent_span->get()) });
  opentracing::Tracer::Global()->Inject(span->context(), writer);
