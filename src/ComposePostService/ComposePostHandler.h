...
void ComposePostHandler::UploadCreator(  
  ...
  auto span = opentracing::Tracer::Global()->StartSpan("UploadCreator",{ opentracing::ChildOf(parent_span->get()) });
  ...
  auto hset_reply = redis_client->hset(std::to_string(req_id),"creator", creator_str);
  auto hlen_reply = redis_client->hincrby(std::to_string(req_id),"num_components", 1);
  redis_client->expire(std::to_string(req_id), REDIS_EXPIRE_TIME);
  redis_client->sync_commit();
  auto timetaken_mongo_call = std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::high_resolution_clock::now()-started_mongo_find).count();
  ...
  if (num_components_reply.as_integer() == NUM_COMPONENTS) {
    ...
    _ComposeAndUpload(req_id, writer_text_map);
     timetaken_mongo_call = std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::high_resolution_clock::now()-started_mongo_find).count();
    timetaken_by_external_calls = timetaken_by_external_calls + timetaken_mongo_call;
  }
...
void ComposePostHandler::UploadText(
  ...
  TextMapWriter writer(writer_text_map);
  auto parent_span = opentracing::Tracer::Global()->Extract(reader);
  auto span = opentracing::Tracer::Global()->StartSpan("UploadText",{ opentracing::ChildOf(parent_span->get()) });
  opentracing::Tracer::Global()->Inject(span->context(), writer);
...
void ComposePostHandler::UploadMedia(
  ...
  TextMapWriter writer(writer_text_map);
  auto parent_span = opentracing::Tracer::Global()->Extract(reader);
  auto span = opentracing::Tracer::Global()->StartSpan("UploadMedia", { opentracing::ChildOf(parent_span->get()) });
  opentracing::Tracer::Global()->Inject(span->context(), writer);
  ...
  //     "RedisHashSet", {opentracing::ChildOf(&span->context())});
  auto started_mongo_find = std::chrono::high_resolution_clock::now();
  auto hset_reply = redis_client->hset(std::to_string(req_id),"media", media_str);
  auto hlen_reply = redis_client->hincrby(std::to_string(req_id),"num_components", 1);
  redis_client->expire(std::to_string(req_id), REDIS_EXPIRE_TIME);
  redis_client->sync_commit();
  // add_span->Finish();
...
void ComposePostHandler::UploadUniqueId(
  ...
  TextMapWriter writer(writer_text_map);
  auto parent_span = opentracing::Tracer::Global()->Extract(reader);
  auto span = opentracing::Tracer::Global()->StartSpan("UploadUniqueId",{ opentracing::ChildOf(parent_span->get()) });
  opentracing::Tracer::Global()->Inject(span->context(), writer);
...
void ComposePostHandler::UploadUrls(
  ...
  TextMapWriter writer(writer_text_map);
  auto parent_span = opentracing::Tracer::Global()->Extract(reader);
  auto span = opentracing::Tracer::Global()->StartSpan("UploadUrls",{ opentracing::ChildOf(parent_span->get()) });
  opentracing::Tracer::Global()->Inject(span->context(), writer);
...
void ComposePostHandler::UploadUserMentions(
  ...
  TextMapWriter writer(writer_text_map);
  auto parent_span = opentracing::Tracer::Global()->Extract(reader);
  auto span = opentracing::Tracer::Global()->StartSpan("UploadUserMentions",{ opentracing::ChildOf(parent_span->get()) });
  opentracing::Tracer::Global()->Inject(span->context(), writer);
  ...
  //     "RedisHashSet", {opentracing::ChildOf(&span->context())});
  auto started_mongo_find = std::chrono::high_resolution_clock::now();
  auto hset_reply = redis_client->hset(std::to_string(req_id),"user_mentions", user_mentions_str);
  auto hlen_reply = redis_client->hincrby(std::to_string(req_id),
                                          "num_components", 1);
  redis_client->expire(std::to_string(req_id), REDIS_EXPIRE_TIME);
  redis_client->sync_commit();
...
void ComposePostHandler::_ComposeAndUpload(
  ...
  auto f_post_id_reply = redis_client->hget(std::to_string(req_id), "post_id");
  auto f_urls_reply = redis_client->hget(std::to_string(req_id), "urls");
  auto f_user_mentions_reply = redis_client->hget(std::to_string(req_id), "user_mentions");
  auto f_post_type_reply = redis_client->hget(std::to_string(req_id), "post_type");
  redis_client->sync_commit();
