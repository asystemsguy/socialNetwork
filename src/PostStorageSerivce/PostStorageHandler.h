...
void PostStorageHandler::StorePost(
  ...
  auto span = opentracing::Tracer::Global()->StartSpan( "StorePost",{ opentracing::ChildOf(parent_span->get()) });
...
void PostStorageHandler::ReadPosts(
  ...
  while (true) {
    ...
    started_mongo_find = std::chrono::high_resolution_clock::now();
    return_value = memcached_fetch(memcached_client, return_key, &return_key_length, &return_value_length, &flags, &memcached_rc);
    timetaken_mongo_call = std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::high_resolution_clock::now()-started_mongo_find).count();
    timetaken_by_external_calls = timetaken_by_external_calls + timetaken_mongo_call;
