...
void SocialGraphHandler::Follow(
  ...
  auto span = opentracing::Tracer::Global()->StartSpan("Follow",{opentracing::ChildOf(parent_span->get())});
...
void SocialGraphHandler::GetFollowers(
  ...
  TextMapWriter writer(writer_text_map);
  auto parent_span = opentracing::Tracer::Global()->Extract(reader);
  auto span = opentracing::Tracer::Global()->StartSpan("GetFollowers",{opentracing::ChildOf(parent_span->get())});
  opentracing::Tracer::Global()->Inject(span->context(), writer);
...
void SocialGraphHandler::GetFollowees(
  ...
  TextMapWriter writer(writer_text_map);
  auto parent_span = opentracing::Tracer::Global()->Extract(reader);
  auto span = opentracing::Tracer::Global()->StartSpan("GetFollowees", {opentracing::ChildOf(parent_span->get())});
  opentracing::Tracer::Global()->Inject(span->context(), writer);
  ...
  //     "RedisGet", {opentracing::ChildOf(&span->context())});
  auto started_mongo_find = std::chrono::high_resolution_clock::now();
  auto num_followees = redis_client->zcard(std::to_string(user_id) + ":followees");
  redis_client->sync_commit();
  auto timetaken_mongo_call = std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::high_resolution_clock::now()-started_mongo_find).count();
  timetaken_by_external_calls = timetaken_by_external_calls + timetaken_mongo_call; 
...
void SocialGraphHandler::InsertUser(
  ...
  TextMapWriter writer(writer_text_map);
  auto parent_span = opentracing::Tracer::Global()->Extract(reader);
  auto span = opentracing::Tracer::Global()->StartSpan("InsertUser", {opentracing::ChildOf(parent_span->get())}); opentracing::Tracer::Global()->Inject(span->context(), writer);
...
void SocialGraphHandler::FollowWithUsername(
  ...
  TextMapWriter writer(writer_text_map);
  auto parent_span = opentracing::Tracer::Global()->Extract(reader);
  auto span = opentracing::Tracer::Global()->StartSpan("FollowWithUsername", {opentracing::ChildOf(parent_span->get())});
...
// Initialize a span
  ...
  TextMapWriter writer(writer_text_map);
  auto parent_span = opentracing::Tracer::Global()->Extract(reader);
  auto span = opentracing::Tracer::Global()->StartSpan("UnfollowWithUsername", {opentracing::ChildOf(parent_span->get())});
  opentracing::Tracer::Global()->Inject(span->context(), writer);
