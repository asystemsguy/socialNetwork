...
void UserTimelineHandler::WriteUserTimeline(
  ...
  bool updated = mongoc_collection_find_and_modify(collection, query, nullptr, update, nullptr, false, true,true, &reply, &error);
...
void UserTimelineHandler::ReadUserTimeline(
  ...
  if (mongo_start < stop) {
    ...
    if (found) {
      ...
      while (bson_iter_find_descendant(&iter_0,
        ...
        redis_update_map.insert(std::make_pair(std::to_string(curr_timestamp),std::to_string(curr_post_id)));
        bson_iter_init(&iter_0, doc);
        bson_iter_init(&iter_1, doc);
        idx++;
      }
  ...
  double timetaken = std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::high_resolution_clock::now()-startt).count();
  span->Log({{"runtime_ms",timetaken-timetaken_by_external_calls}});
  span->Finish();
